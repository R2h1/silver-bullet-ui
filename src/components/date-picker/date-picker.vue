<template>
  <div :class="`${prefixClass}`">
    <span v-if="showLabel && label" :class="`${prefixClass}__label`">{{
      label
    }}</span>
    <div :class="`${prefixClass}__content`">
      <div
        :class="`${prefixClass}__content-begin`"
        :style="{
          '--input-inner-width': startInputWidth,
        }"
        @mouseenter="mouseenterStart = true"
        @mouseleave="mouseenterStart = false"
      >
        <prefix-icon
          v-if="showPrefixIcon"
          @click.native="$refs['start-date-picker'].focus()"
        ...
        <suffix-icon
          v-if="showSuffixIcon"
          v-show="!showStartClearIcon"
          :class="`${prefixClass}__suffix`"
        />
        <clear-icon
          @click.native="clear(startField)"
          v-show="showStartClearIcon"
          :class="`${prefixClass}__clear-icon`"
        />
      </div>
      <span
        v-if="value[endField] != null"
        :class="`${prefixClass}__content-separator`"
        >{{ separator }}</span
      >
      <div
        v-if="value[endField] != null"
        :class="`${prefixClass}__content-end`"
        :style="{ '--input-inner-width': endInputWidth }"
        @mouseenter="mouseenterEnd = true"
        @mouseleave="mouseenterEnd = false"
      >
        <prefix-icon
          v-if="showPrefixIcon"
          @click.native="$refs['end-date-picker'].focus()"
          :class="`${prefixClass}__prefix`"
        />
        <el-date-picker
          ref="end-date-picker"
        ...
        <suffix-icon
          v-if="showSuffixIcon"
          v-show="!showEndClearIcon"
          :class="`${prefixClass}__suffix`"
        />
        <clear-icon
          @click.native="clear(endField)"
          v-show="showEndClearIcon"
          :class="`${prefixClass}__clear-icon`"
        />
      </div>
...
import { ClearIcon, SuffixIcon, PrefixIcon } from "../icons";
...
export default {
  ...
  computed: {
    startInputWidth() {
      ...
      return `${
        this.value[this.startField] ? this.dateWidth : this.placeholderWidth
      }px`;
    },
    endInputWidth() {
      if (!this.isInputFitContent) {
      ...
      return `${
        this.value[this.endField] ? this.dateWidth : this.placeholderWidth
      }px`;
    },

    showStartClearIcon() {
      return (
        this.clearable && this.value[this.startField] && this.mouseenterStart
      );
    },
    showEndClearIcon() {
    ...
    modifyMonthCell() {
      this.$nextTick(() => {
        const poppers = Array.from(
          document.body.querySelectorAll(`.${this.prefixClass}__popper`)
        );
        poppers.forEach((popper) => {
          const cells = Array.from(
            popper.querySelectorAll(".el-month-table .cell")
          );
