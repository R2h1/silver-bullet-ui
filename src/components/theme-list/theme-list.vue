  ...
  >
    ...
    >
      <span :class="`${prefixClass}__item--can-drag`">
        <move-icon
          class="move-icon"
          v-if="canDraggable && !item.disabled && !!item.draggable"
        />
        {{ item.label }}
      </span>
      <el-switch v-if="multiple" :value="activeList[index]"> </el-switch>
    </li>
  </transition-group>
</template>
<script>
  ...
  methods: {
    handleClick(value, index) {
      if (this.multiple) {
        ...
        if (isSelected) {
          this.value.splice(
            this.value.findIndex((item) => item === value),
            1
          );
        } else {
      ...
      if (this.once && value[this.valueField] === this.value[this.valueField])
        return;
