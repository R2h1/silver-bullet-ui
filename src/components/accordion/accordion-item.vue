<template>
  <div :class="`${prefixClass}`" :style="styles" :data-active="active">
    <button :class="`${prefixClass}__control`" :data-accordion-control="true" @click="handleClick(data)">
      <span :class="`${prefixClass}__control--chevron`" :data-rotate="active">
        <svg viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg" style="width:16px; height: 16px; display:block">
    ...
    <div ref="panel" :class="`${prefixClass}__panel`">
      <div :class="`${prefixClass}__panel--content`">{{data.description}}</div>
    </div>
  </div>
</template>
